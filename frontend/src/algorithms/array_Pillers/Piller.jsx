...
export default class Piller extends Component {
    render() {
        const {
            val,
            isChanging,
        ...
        return (
            <>
                <div className={`pile ${extraClassName}`} style={{ height: `${val * heights[size]}px`, background: color }}>
                    <p className='value'>{val}</p>
                    {!finished && isPivot && <svg height="100" width={indicatorLength[size] + 100} style={{ position: "absolute", display: "flex", marginTop: "-31px", marginLeft: `${lineOff}px` }}>
